...
export default function Sidebar({ active, onNav, wsStatus, alerts, user, onLogout }) {
  ...
  const roleColor = user?.role === 'admin' ? 'var(--green)' : 'var(--cyan)'
  ...
  return (
    <aside style={{
      ...
      <nav style={{ flex: 1, padding: '12px 8px', overflow: 'auto' }}>
        {NAV_ITEMS.map(({ id, icon: Icon, label }) => {
          ...
          return (
            ...
            >
              <Icon size={18} style={{ flexShrink: 0, filter: isActive ? 'drop-shadow(0 0 6px var(--cyan))' : 'none' }} />
              {!collapsed && (
                <span style={{ fontFamily: 'var(--font-ui)', fontSize: 13, fontWeight: isActive ? 600 : 400 }}>
                  {label}
                </span>
              )}
      ...
      <div style={{
        ...
        <User size={14} color={roleColor} style={{ flexShrink: 0 }} />
        {!collapsed && (
      ...
      <div style={{
        ...
        {!collapsed && (
          <span style={{
            fontFamily   : 'var(--font-mono)',
            fontSize     : 10,
            color        : wsStatus === 'connected' ? 'var(--green)' : 'var(--text-muted)',
            letterSpacing: '0.05em',
          }}>
